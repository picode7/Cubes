      ...
      for (let sy of sx) {
        for (let sub of sy) {
          if (sub != null) {
            this.geom.merge(
              sub.geom,
              new THREE.Matrix4().setPosition(new THREE.Vector3(sub.position.x, sub.position.y, sub.position.z))
            )
          }
        }
      }
...
class Cube {
  ...
  buildGeometry() {
    ...
    let face = (neighbor: Cube) => {
      return neighbor == null || (neighbor.type == CUBE_TYPE.glass && this.type != CUBE_TYPE.glass)
    }
    ...
    // Vertices
    let vbbl: number, vbfl: number, vbfr: number, vbbr: number, vtbl: number, vtfl: number, vtfr: number, vtbr: number
  ...
  remove() {
    ...
    if (this.position.y == game.world.lowestPoint) {
      ...
      for (let cube of game.world.cubes) {
        if (cube.position.y < game.world.lowestPoint) game.world.lowestPoint = cube.position.y
      }
    }
  ...
  checkNeighbors(updateOthers = false) {
